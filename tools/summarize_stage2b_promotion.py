"""Validate and compare the paired S2-A/S2-B promotion-screen outputs."""
...
EXPECTED_S2A_SHA256 = (
    "f1f218306edda84f7d1ef1d06944435c683b6747604cf9e9f6e1ab0f9aed9eb2"
)
EXPECTED_S2B_SHA256 = (
...
class PromotionSummaryConfig:
    candidate_dir: str
    """Output directory from the S2-B candidate evaluation."""

    baseline_dir: str
    """Output directory from the S2-A baseline evaluation."""
...
def _expanded(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    selected = [
        row for row in rows
        if row["protocol_phase"] == "promotion_expanded_5mm"
    ]
...
def _make_figure(
    *,
    ...
    baseline_rate = _phase_rate(baseline_expanded, "promotion_expanded_5mm")
    candidate_rate = _phase_rate(candidate_expanded, "promotion_expanded_5mm")
    core_rate = _phase_rate(candidate_rows, "promotion_core_2mm")
    ...
    axes[0].axhline(0.88, color="#2e7d32", linestyle=":", linewidth=1.4)
    ...
    axes[1].set_title("S2-B minus S2-A at paired ±5 mm positions")
    ...
    png = output_dir / "s2b_promotion_comparison.png"
    svg = output_dir / "s2b_promotion_comparison.svg"
...
def summarize(cfg: PromotionSummaryConfig) -> Path:
        ...
        paired_rows.append(
            {
                ...
                "s2a_baseline_success": baseline_success,
                "s2b_candidate_success": candidate_success,
    ...
    paired_csv = output_dir / "paired_expanded_5mm_conditions.csv"
    ...
    comparison = {
        ...
        "candidate_expanded_5mm_successes": candidate_successes,
        "candidate_expanded_5mm_success_rate": candidate_rate,
        "candidate_expanded_5mm_wilson_95_lower": candidate_lower,
        "candidate_expanded_5mm_wilson_95_upper": candidate_upper,
        "baseline_expanded_5mm_successes": baseline_successes,
        "baseline_expanded_5mm_success_rate": baseline_rate,
        "baseline_expanded_5mm_wilson_95_lower": baseline_lower,
        "baseline_expanded_5mm_wilson_95_upper": baseline_upper,
        ...
        "checkpoint_s2b_promotion_gate_pass": promotion_pass,
        "gate_definition": {
        ...
        "interpretation_note": (
            "Candidate and baseline used the same distinct +/-5 mm positions. "
    ...
    print(f"[S2-B promotion] Paired conditions: {total}")
    print(f"[S2-B promotion] Position mismatches: {position_mismatches}")
    print(f"[S2-B promotion] S2-A +/-5 mm: {100.0 * baseline_rate:.2f}%")
    print(f"[S2-B promotion] S2-B +/-5 mm: {100.0 * candidate_rate:.2f}%")
    print(f"[S2-B promotion] Improvement: {100.0 * improvement:+.2f} pp")
    print(f"[S2-B promotion] Smoke validation: {smoke_validation_pass}")
    print(f"[S2-B promotion] Final gate: {promotion_pass}")
    print(f"[S2-B promotion] Summary: {summary_path}")
    print(f"[S2-B promotion] Figure: {figure_paths[0]}")
    return output_dir
