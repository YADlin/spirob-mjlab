"""Compact frozen-policy promotion screen for SpiRob Stage 2-C.

This evaluator answers only the curriculum-promotion question:

* candidate: did the S2-C checkpoint retain +/-5 mm and acquire +/-10 mm?
* baseline: how does the S2-B checkpoint perform at the exact same +/-10 mm
  continuous positions?

The full candidate protocol contains 50 nominal repeats, 625 unique balanced
+/-5 mm positions, and 625 unique balanced +/-10 mm positions.  The full
baseline protocol contains the same 625 +/-10 mm positions used for the
...
STAGE2C_TASK_ID = "Mjlab-SpiRob-EggToBucket-Stage2C"

EVALUATION_SEED = 2510
CORE_POSITION_SEED = 2505
EXPANDED_POSITION_SEED = 2510
STRATA_PER_AXIS = 5
...
class PromotionEvaluateConfig:
    """Inputs for one checkpoint in the paired S2-C promotion screen."""

    run_id: str
    """Scientific identifier for this evaluation."""

    role: Role
    """Candidate is S2-C; baseline is the accepted S2-B checkpoint."""
...
def _make_summary(
    ...
    if cfg.role == "candidate":
        ...
        phase_summaries["core_5mm"] = core_summary
    ...
    phase_summaries["expanded_10mm"] = expanded_summary
    ...
    if cfg.role == "candidate" and cfg.mode == "full" and complete:
        core = phase_summaries["core_5mm"]
        expanded = phase_summaries["expanded_10mm"]
    ...
    return (
        {
            ...
            "gate_definition": {
                ...
                "candidate_improvement_over_s2b_baseline_minimum": (
...
def run_evaluation(cfg: PromotionEvaluateConfig) -> Path:
    ...
    task_id = STAGE2C_TASK_ID if cfg.role == "candidate" else STAGE2B_TASK_ID
    ...
    try:
        with csv_path.open("w", newline="", encoding="utf-8") as stream:
            ...
            for batch_number, start in enumerate(
                ...
                print(
                    f"[S2-C promotion:{cfg.role}] batch "
    ...
    metadata = {
        ...
        "checkpoint_size_bytes": checkpoint.stat().st_size,
        "position_seeds": {
            "core_5mm": CORE_POSITION_SEED,
            "paired_expanded_10mm": EXPANDED_POSITION_SEED,
        },
    ...
    print(f"[S2-C promotion:{cfg.role}] Episodes: {csv_path}")
    print(f"[S2-C promotion:{cfg.role}] Summary: {summary_path}")
    print(f"[S2-C promotion:{cfg.role}] Checkpoint SHA-256: {checkpoint_hash}")
    print(
        f"[S2-C promotion:{cfg.role}] Protocol complete: "
        f"{summary['protocol_complete']}"
    )
    print(
        f"[S2-C promotion:{cfg.role}] Candidate component gate: "
