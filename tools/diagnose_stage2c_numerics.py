...
contained the NaN episodes in the archived workspace map.  It then runs 500
repeats at each implicated position and at adjacent and mirrored controls.
...
be inspected instead of merely counted.
"""
...
def _available_raw_state(
    ...
    for name in RAW_STATE_NAMES:
        ...
        if (
            ...
            and value.ndim >= 2
            and value.shape[0] >= active_count
        ):
            available[name] = value[:active_count]
    return available
